...
template <typename T> struct dataiter {
	T *begin_, *end_;
	dataiter(char *begin, std::size_t n) : begin_(reinterpret_cast<T *>(begin)), end_(begin_ + n) {}
	dataiter(const char *begin, std::size_t n)
...
/// helper friend functions to avoid defining dataiter in sample.h
inline char *lsl::iterhelper(sample &s) { return reinterpret_cast<char *>(&s.data_); }
inline const char *lsl::iterhelper(const sample &s) {
	return reinterpret_cast<const char *>(&s.data_);
}
...
inline void copyconvert_array(const T *src, U *dst, std::size_t n) noexcept {
	for (const T *end = src + n; src < end;) *dst++ = static_cast<U>(*src++);
}
...
template <> inline void endian_reverse_inplace(double &t) {
	endian_reverse_inplace(*reinterpret_cast<uint64_t *>(&t));
}
template <> inline void endian_reverse_inplace(float &t) {
	endian_reverse_inplace(*reinterpret_cast<uint32_t *>(&t));
}
	...
	if (format_ == cft_string) {
		for (const auto &str : samplevals<std::string>(*this)) {
			...
			if (str.size() <= 0xFF) {
				...
				save_value(sb, (uint8_t)str.size(), use_byte_order);
			} else {
				if (str.size() <= 0xFFFFFFFF) {
					save_value(sb, (uint8_t)sizeof(uint32_t), use_byte_order);
					save_value(sb, (uint32_t)str.size(), use_byte_order);
				} else {
					save_value(sb, (uint8_t)sizeof(uint64_t), use_byte_order);
					save_value(sb, static_cast<uint64_t>(str.size()), use_byte_order);
				}
			}
...
void lsl::sample::convert_endian(void *data, uint32_t n, uint32_t width) {
	...
	case 1: break;
	case sizeof(int16_t):
		for (auto &val : dataiter<int16_t>(cdata, n)) endian_reverse_inplace(val);
		break;
	case sizeof(int32_t):
		for (auto &val : dataiter<int32_t>(cdata, n)) endian_reverse_inplace(val);
		break;
	case sizeof(int64_t):
		for (auto &val : dataiter<int64_t>(cdata, n)) endian_reverse_inplace(val);
		break;
